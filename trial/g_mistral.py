...
def initialize_environment():
    nest_asyncio.apply()
    os.environ["MISTRAL_API_KEY"] = "JnHS6BgHZ7gaO2ipKN1qAbYwSJXu4oNq"
...
def process_batches(
    ...
    for i in range(0, len(df_knowledge), batch_size):
        ...
        batch_df = df_knowledge.iloc[i:i + batch_size].copy()

        # Use both 'summary' and 'text' as separate columns
        kb = KnowledgeBase.from_pandas(batch_df, columns=["summary", "text"])

        try:
            testset = safe_generate_testset(
                kb,
...
def main():
    ...
    output_path = "/home/shtlp_0010/Desktop/giskard_dataset/generated.json"
    ...
    # Combine both columns into one for knowledge base usage
    df["content"] = df["summary"].fillna('') + " " + df["text"].fillna('')
